...
class TestDataConverter(unittest.TestCase):
    ...
    def test_update_data_structure(self):
        ...
        expected_data = {
            "Year 2": {
                "semester 2": {"C002": {"title": "Physics", "unit": "4", "grade": "B"}}
            }
        }
        ...
        self.converter._update_data_structure(row)
        expected_data["Year 2"]["semester 2"]["C003"] = {
            "title": "Chemistry",
            "unit": "4",
            "grade": "C",
        }
        ...
        self.converter._update_data_structure(row)
        expected_data["Year 2"]["semester 2"]["C002"] = {
            "title": "Physics II",
            "unit": "3",
            "grade": "A",
        }
    ...
    def test_validate_grade_input(self):
        ...
        with patch("builtins.input", side_effect=["V", "c"]), patch(
        ...
        # Case 4: Valid input followed by extra characters
        mock_input = ["ELONISGOD", "A"]
        with unittest.mock.patch("builtins.input", side_effect=mock_input), patch(
            "sys.stdout", new_callable=io.StringIO
        ) as mock_obj:
            grade = self.converter._validate_grade_input("Enter grade: ")
        self.assertEqual(grade, "A")
        ...
        # Case 5: Enter key pressed or empty string
        mock_input = ["", "A"]
        with unittest.mock.patch("builtins.input", side_effect=mock_input), patch(
            "sys.stdout", new_callable=io.StringIO
        ) as mock_obj:
    ...
    def test_convert_raw_template(self):
        ...
        with patch("builtins.input", side_effect=["student_result", "A"]), patch(
            "sys.stdout", new_callable=io.StringIO
        ) as mock_obj:
        ...
        self.assertEqual(
            self.converter.data,
            {
                "Year 1": {
                    "semester 1": {
                        "EGR 101": {"title": "Math", "unit": "2", "grade": "A"}
                    }
                }
            },
        )
    ...
    def test_convert_complete_template(self):
        ...
        self.assertEqual(
            mock_stdout.getvalue(), "Data has been written to template_file\n"
        )
    ...
    def test_write_to_json(self):
        self.converter.data = {
            "Year 1": {
                "semester 1": {"C001": {"title": "Math", "unit": "3", "grade": "A"}}
            }
        }
        with patch("sys.stdout", new_callable=io.StringIO), patch(
            "builtins.open", create=True
